  ...
  getAuthorizer(http, functionName) {
    ...
    if (typeof authorizer === 'string') {
      if (authorizer.indexOf(':') === -1) {
        ...
        arn = { 'Fn::GetAtt': [`${normalizedName}LambdaFunction`, 'Arn'] };
      } else {
        arn = authorizer;
        const splitArn = arn.split(':');
        const splitLambdaName = splitArn[splitArn.length - 1].split('-');
        name = splitLambdaName[splitLambdaName.length - 1];
      }
      ...
      if (authorizer.arn) {
        ...
        const normalizedName = authorizer.name[0].toUpperCase() + authorizer.name.substr(1);
        arn = { 'Fn::GetAtt': [`${normalizedName}LambdaFunction`, 'Arn'] };
