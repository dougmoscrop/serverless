...
class AwsProvider {
  ...
  request(service, method, params, options) {
    ...
    const region = _.get(options, 'region', this.getRegion());
    ...
    const paramsWithRegion = _.merge({}, params, { region });
  ...
  /**
   * Uses a credentials provider chain to load credentials from several well-known sources
   * @returns AWS.Credentials
   */
  getCredentials() {
    if (!this.cachedCredentials) {
      const credentials = new AwsCredentials();
      const stageUpperCase = this.getStage().toUpperCase();
      ...
      if (this.serverless.service.provider.profile && !this.options['aws-profile']) {
        credentials.addProfile(this.serverless.service.provider.profile); // config profile
      }

      credentials.addConfig(this.serverless.service.provider.credentials); // config creds

      credentials.addProfile('default');
